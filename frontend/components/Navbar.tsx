...
export default function Navbar() {
  ...
  const getDashboardLink = () => {
    switch(userRole) {
      ...
      case 'student': return '/subjects';
      default: return '/';
    }
  };

  return (
    <nav className="bg-gradient-to-r from-blue-700 to-indigo-800 text-white shadow-md mb-6">
      <div className="container mx-auto px-4 py-3">
        <div className="flex justify-between items-center">
          ...
          {userRole && (
            <div className="hidden md:flex items-center space-x-6">
              <Link href={getDashboardLink()} className="nav-link font-medium">
                {userRole === 'student' ? 'My Subjects' : 'Dashboard'}
              </Link>
              
              {isAdmin && (
                <Link href="/admin-dashboard/subjects" className="nav-link font-medium">
                  Manage Subjects
                </Link>
              )}
              
              <div className="flex items-center ml-4">
                <span className="mr-3 text-sm opacity-90 bg-blue-800 px-2 py-1 rounded">
                  <span className="capitalize">{userRole}</span>: {userName}
                </span>
                <button 
        ...
        {/* Mobile Menu */}
        {isMenuOpen && userRole && (
          <div className="md:hidden pt-4 pb-2 border-t border-blue-600 mt-3">
            <div className="flex flex-col space-y-3">
              <Link 
                href={getDashboardLink()} 
                className="py-2 px-1 hover:bg-blue-800 rounded transition-colors duration-200"
