...
export default function ReportsPage() {
  ...
  const fetchReports = async () => {
    try {
      ...
      const response = await fetch('http://localhost:5001/api/dean/reports', {
        headers: {
  ...
  const fetchDetailedFeedback = async (subjectId: string) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`http://localhost:5001/api/dean/feedback/${subjectId}/details`, {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
        },
      ...
      showToast('Failed to load detailed feedback', 'error');
    } finally {
  ...
  const filteredReports = reports.filter(report => {
    const matchesSearch = report.subject.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
                         report.subject.code.toLowerCase().includes(searchTerm.toLowerCase()) ||
                         report.faculty.name.toLowerCase().includes(searchTerm.toLowerCase());
  ...
  const uniqueBranches = Array.from(new Set(reports.map(r => r.subject.branch.code)));
          ...
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {filteredReports.map((report) => (
              <div key={report._id} className="bg-white rounded-lg shadow-md p-6 border border-gray-200">
                <div className="mb-4">
                  <h3 className="text-lg font-semibold text-gray-900 mb-1">{report.subject.name}</h3>
                  <p className="text-sm text-gray-500 font-mono">{report.subject.code}</p>
                  <p className="text-sm text-gray-600">{report.subject.branch.name}</p>
                </div>

                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-gray-600">Faculty</span>
                    <span className="text-sm font-medium text-gray-900">{report.faculty.name}</span>
                  </div>

                  <div className="flex items-center justify-between">
                    <span className="text-sm text-gray-600">Responses</span>
                    <span className="text-sm font-medium text-gray-900">{report.totalResponses}</span>
                  </div>
                      ...
                      <div className="flex">
                        {[...Array(5)].map((_, i) => (
                          <svg
                            key={i}
                            className={`h-3 w-3 ${
                              i < Math.floor(report.averageRating) ? 'text-yellow-400' : 'text-gray-300'
                            }`}
                            fill="currentColor"
                            viewBox="0 0 20 20"
                          >
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${getResponseRateColor(report.responseRate)}`}>
